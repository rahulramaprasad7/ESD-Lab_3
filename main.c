...
void deleteNode(struct buff **head_ref)
{
    ...
    while (temp != NULL && temp->bufferNo != position)
    {
        prev = temp;
        temp = temp->next;
    }
...
void main()
{
    ...
    do
    {
        ...
        buffer_0->freeCharacters = buffer_0->size - i;                          //Calculate free space
    ...
    }while (1);                                                        //Continue reading serial inputs until "Enter" key is pressed
}
int putchar (int c)
{
